...
let cardIndex = 0;
let cardsPerView = Math.floor(productContainer.offsetWidth / 250); // Calcula las tarjetas visibles al inicio

// Función para crear los círculos de paginación
function createPagination() {
   pagination.innerHTML = ''; // Limpia los círculos anteriores
   cardsPerView = Math.floor(productContainer.offsetWidth / 250); // Recalcula tarjetas visibles
   const totalPages = Math.ceil(productCards.length / cardsPerView); // Ajustar total de páginas
...
// Actualizar los círculos de paginación
function updatePagination() {
   const dots = document.querySelectorAll('.dot');
   dots.forEach(dot => dot.classList.remove('active'));
   const activeDot = Math.round(productContainer.scrollLeft / productContainer.clientWidth); // Usar el ancho completo para activar el círculo
   if (dots[activeDot]) {
      dots[activeDot].classList.add('active');
   }
}

// Evento scroll para actualizar los puntos en tiempo real
productContainer.addEventListener('scroll', () => {
   updatePagination();
});

// Clic en los círculos de paginación
pagination.addEventListener('click', (e) => {
   if (e.target.classList.contains('dot')) {
      const index = parseInt(e.target.dataset.index);
      const scrollAmount = index * productContainer.clientWidth; // Moverse al ancho de una página completa
...
// Listeners para los botones de siguiente y anterior
nextBtn.addEventListener('click', () => moveCarousel(productContainer.clientWidth));
preBtn.addEventListener('click', () => moveCarousel(-productContainer.clientWidth));

// Recalcular y redibujar los círculos de paginación cuando la ventana se redimensione
window.addEventListener('resize', createPagination);

// Inicializa la paginación al cargar la página
createPagination();

//FIN carrusel de ropa 
...
function applyContrast(level) {
    ...
    if (level === "high") {
        document.body.style.backgroundImage = "url('fondo-pag-negro.jpg')";
    } else {
        document.body.style.backgroundImage = "url('fondo-pag.jpg')";
    }
