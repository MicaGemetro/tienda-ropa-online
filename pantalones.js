//funcion de cambio de colores de los circulos e imagenes

document.addEventListener("DOMContentLoaded", function () {
    // Agregar evento click al contenedor de pantalones
    document.querySelector(".pantalones-grid").addEventListener("click", function (event) {
        // Verificar si el clic fue en un elemento de color
        const colorElemento = event.target.closest(".color");

        if (colorElemento) {
            // Encontrar el contenedor del pantalón correspondiente
            const contenedorPantalon = colorElemento.closest(".pantalon-container");

            // Extraer el color seleccionado
            const color = colorElemento.dataset.color;

            // Cambiar la imagen del pantalón
            const imagen = contenedorPantalon.querySelector(".pantalon-img");
            imagen.src = `vestimenta/pantalones/${color}.jpg`;
            imagen.alt = `Pantalón ${color.charAt(0).toUpperCase() + color.slice(1)}`;

            // Cambiar el color seleccionado
            const colores = contenedorPantalon.querySelectorAll(".color");
            colores.forEach(function (color) {
                color.classList.remove("seleccionado");
            });

            // Marcar el nuevo color como seleccionado
            colorElemento.classList.add("seleccionado");

            // Actualizar el enlace al detalle del pantalón con el color seleccionado
            const linkPantalon = contenedorPantalon.querySelector(".pantalon-link");
            linkPantalon.href = `pantalon-corto.html?color=${color}`;
        }
    });
});
...
// funcion para cambiar de color las imagenes de los pantalones
function cambiarPantalon(color, precio, id) {
    // Cambiar la imagen del pantalón específico
    const imagen = document.getElementById(`pantalon-imagen-${id}`);
    imagen.src = `vestimenta/pantalones/${color}.jpg`;
    imagen.alt = `Pantalón ${color.charAt(0).toUpperCase() + color.slice(1)}`;

    // Cambiar el nombre del pantalón
    //const nombre = document.getElementById(`pantalon-nombre-${id}`);
    //nombre.textContent = `Pantalón ${color.charAt(0).toUpperCase() + color.slice(1)}`;

    // Cambiar el precio del pantalón
    const precioElemento = document.getElementById(`pantalon-precio-${id}`);
    precioElemento.innerHTML = `<b>${precio}</b>`; // Usa innerHTML para incluir la etiqueta <b>

    // Actualizar el círculo seleccionado solo en el pantalón correspondiente
    const contenedor = document.getElementById(`pantalon-${id}`);
    const colores = contenedor.querySelectorAll(".color");
    colores.forEach(circulo => {
        circulo.classList.remove("seleccionado"); // Quitar la clase 'seleccionado' de todos
    });

    // Agregar la clase al círculo seleccionado
    const circuloSeleccionado = contenedor.querySelector(`.color[data-color="${color}"]`);
    if (circuloSeleccionado) {
        circuloSeleccionado.classList.add("seleccionado");
    }
} 
